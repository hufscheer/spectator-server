		...
		result.andExpect((status().isOk()))
			.andDo(restDocsHandler.document(
				...
				responseFields(
					...
					fieldWithPath("[].gameTeamPlayers[].id").type(JsonFieldType.NUMBER).description("선수 ID"),
					...
					fieldWithPath("[].gameTeamPlayers[].state").type(JsonFieldType.STRING).description("선수의 선발 상태 ")
				)
			));
	}
}
