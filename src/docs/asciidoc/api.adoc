...
선수의 `position` 은 **선발 전원의 포지션이 등록된 경우에만** 내려간다. 한 명이라도 비어 있으면
후보를 포함한 라인업 전체가 `null` 이며, 이때는 등번호와 이름만 표시하면 된다.
일부 선수만 포지션이 보이는 불완전한 라인업을 막기 위한 정책이다.

판정 기준은 출전 여부가 아니라 선발 여부다. 포지션이 없는 후보가 교체 투입돼도 표시가 사라지지 않는다.

`starterPlayers` 는 포지션을 내려보낼 때 축구 FW → MF → DF → GK, 농구 PG → SG → SF → PF → C 순으로
정렬되고, 그렇지 않으면 등번호 순이다.
...
선수의 포지션은 선수 개인이 아니라 **팀에서의 정보**라 팀 선수 등록·수정 요청으로 다룬다.
같은 선수가 축구팀에서는 `LW`, 농구팀에서는 `PG` 일 수 있다. 선택 입력이라 생략하거나 `null` 로 보내면 해제된다.

- 축구: `GK` `LB` `CB` `RB` `LM` `CM` `RM` `LW` `ST` `RW`
- 농구: `PG` `SG` `SF` `PF` `C`

팀 종목과 맞지 않는 포지션을 보내면 400 이다. 경기 라인업에는 등번호와 마찬가지로 로스터 값이 자동 복사되므로
경기마다 다시 입력하지 않는다. 이미 만들어진 라인업은 이후 로스터에서 포지션을 바꿔도 그대로 유지된다.
